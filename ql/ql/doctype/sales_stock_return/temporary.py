...
class SalesStockReturn(SellingController):
	def __init__(self, *args, **kwargs):
		super(SalesStockReturn, self).__init__(*args, **kwargs)
		self.is_return = 1
		self.shipping_address_name = ''
		self.status_updater = [{
			'source_dt': 'Sales Stock Return Item',
		...
		if cint(self.is_return):
			self.status_updater.append({
				...
				'second_join_field': 'so_detail',
				'extra_cond': """ and exists (select name from `tabSales Stock Return`
					where name=`tabSales Stock Return Item`.parent and is_return=1)""",
				'second_source_extra_cond': """ and exists (select name from `tabSales Invoice`
	...
	def before_print(self):
		...
		item_meta = frappe.get_meta("Sales Stock Return Item")
		print_hide_fields = {
		...
		super(SalesStockReturn, self).before_print()
	...
	def validate(self):
		...
		super(SalesStockReturn, self).validate()
		self.set_status()
		self.so_required()
	...
	def validate_with_previous_doc(self):
		super(SalesStockReturn, self).validate_with_previous_doc({
			"Sales Order": {
	...
	def validate_warehouse(self):
		super(SalesStockReturn, self).validate_warehouse()
	...
	def on_cancel(self):
		super(SalesStockReturn, self).on_cancel()
	...
	def check_next_docstatus(self):
		submit_rv = frappe.db.sql("""select t1.name
			...
			where t1.name = t2.parent and t2.sales_stock_return = %s and t1.docstatus = 1""",
			(self.name))
		if submit_rv:
	...
	def cancel_packing_slips(self):
		"""
			Cancel submitted packing slips related to this Sales Stock Return
		"""
		res = frappe.db.sql("""SELECT name FROM `tabPacking Slip` WHERE sales_stock_return = %s
	...
	def update_billing_status(self, update_modified=True):
		updated_sales_stock_returns = [self.name]
		for d in self.get("items"):
			if d.si_detail and not d.so_detail:
				d.db_set('billed_amt', d.amount, update_modified=update_modified)
			elif d.so_detail:
				updated_sales_stock_returns += update_billed_amount_based_on_so(d.so_detail, update_modified)

		for dn in set(updated_sales_stock_returns):
			dn_doc = self if (dn == self.name) else frappe.get_doc("Sales Stock Return", dn)
...
def update_billed_amount_based_on_so(so_detail, update_modified=True):
	...
	# Get all Sales Stock Return Item rows against the Sales Order Item row
	dn_details = frappe.db.sql("""select dn_item.name, dn_item.amount, dn_item.si_detail, dn_item.parent
		from `tabSales Stock Return Item` dn_item, `tabSales Stock Return` dn
	...
	for dnd in dn_details:
		...
		if dnd.si_detail:
			...
			# Get billed amount directly against Sales Stock Return
		...
		frappe.db.set_value("Sales Stock Return Item", dnd.name, "billed_amt", billed_amt_agianst_dn, update_modified=update_modified)
...
def get_invoiced_qty_map(sales_stock_return):
	"""returns a map: {dn_detail: invoiced_qty}"""
	invoiced_qty_map = {}

	for dn_detail, qty in frappe.db.sql("""select dn_detail, qty from `tabSales Invoice Item`
		where sales_stock_return=%s and docstatus=1""", sales_stock_return):
...
def get_returned_qty_map(sales_stock_return):
	...
	returned_qty_map = frappe._dict(frappe.db.sql("""select dn_item.dn_detail, abs(dn_item.qty) as qty
		...
		where dn.name = dn_item.parent
			and dn.docstatus = 1
			and dn.is_return = 1
	""", sales_stock_return)) #and dn.return_against = %s
