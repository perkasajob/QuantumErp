...
def get_consumed_items(condition):
	consumed_items = frappe.db.sql("""
		select sle.item_code, abs(sum(sle.actual_qty)) as consumed_qty
		from `tabStock Ledger Entry` sle
		INNER JOIN `tabStock Entry` se ON se.name = sle.voucher_no
		where sle.actual_qty < 0
			and se.stock_entry_type in ("Material Consumption at Subcontractor", "Material Consumption for Manufacture", "Material Issue")
			and sle.%s
...
def get_delivered_items(condition):
	dn_items = frappe.db.sql("""select dn_item.item_code, sum(dn_item.stock_qty) as dn_qty
		...
		where dn.name = dn_item.parent and dn.docstatus = 1 and %s
...
def get_condition(filters):
	...
	if filters.get("from_date") and filters.get("to_date"):
		conditions += " posting_date between '%s' and '%s'" % (filters["from_date"],filters["to_date"])
	else:
